...
def convert_to_serializable(obj):
    """Convert object to JSON serializable format with robust type checking"""
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                if cleaner.df is None or cleaner.df.empty:
                    flash('The uploaded file appears to be empty or could not be read properly. Please check the file format.', 'error')
                    return redirect(request.url)
                
                print(f"DataFrame loaded successfully! Shape: {cleaner.df.shape}")
                ...
                if 'empty' in str(e).lower() or 'no columns' in str(e).lower():
                    flash('The file appears to be empty or in an unexpected format. Please check if it contains valid data.', 'error')
                else:
                    flash(f'Error: {str(e)}', 'error')
                
                return redirect(request.url)
        else:
            flash('File type not allowed. Please upload CSV, Excel, JSON, or TXT files.', 'warning')
            return redirect(request.url)
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("DataPure — Starting Development Server")
    print("="*60)
